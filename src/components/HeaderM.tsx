...
function HeaderM({ pathname, setTheme, lighttheme }: IProps) {
  ...
  const loginOff = () => {
    ...
    setUserLoginState(false)
    LoginAPI.deleteUser(localStorage.getItem('current_user_emailValue'))
  }

  const UserInfoPopoverContent = (
    <div>
      <p className="firstTXT">Hello,dear user</p>
      <p className="secondTXT">{localStorage.getItem('current_user_emailValue')}</p>
    ...
    <>
      <Row gutter={[24, 0]}>
        ...
        <Col span={24} md={18} className="header-control">
          <Popover content={UserInfoPopoverContent} placement="bottom" title="User Info" style={userLoginState ? { display: '' } : { display: "none" }} className="loginPopover">
            <img className="indexAvatar" src={avatar} alt="图片不见了" />
          </Popover>
